...
def draw_chart_with_backtest(df, trades, name):
    ...
    fig.add_trace(go.Scatter(x=df.index, y=df['MA20'], line=dict(color='orange'), name='20일선'))
    ...
    fig.add_trace(go.Scatter(x=buy_x, y=buy_y, mode='markers', marker=dict(color='red', size=10, symbol='triangle-up'), name='매수'))
    fig.add_trace(go.Scatter(x=sell_x, y=sell_y, mode='markers', marker=dict(color='blue', size=10, symbol='triangle-down'), name='매도'))
    fig.update_layout(height=400, title=f"{name} 1년 백테스팅", xaxis_rangeslider_visible=False)
...
def analyze_holding_period(stock_name):
    ...
    volatility = df['Change'].mean() * 100 # 평균 일일 변동폭 (%)
    ...
    char_type = ""
    if volatility > 2.5: char_type = "🚀 모멘텀형 (변동성 큼)"
    elif trend_ratio > 0.6: char_type = "🧱 누적형 (꾸준한 우상향)"
    else: char_type = "🎢 이벤트형 (박스권/불규칙)"
    
    # 4. 보유 기간별 수익률 시뮬레이션
    # (단순 이동평균 돌파 시 매수했다고 가정하고 N일 들고 있기)
    holding_days = [5, 10, 20, 40, 60, 90] # 1주 ~ 3달
    results = {}
    ...
    for days in holding_days:
        ...
        if profit_list:
            results[f"{days//5}주({days}일)"] = np.mean(profit_list)
    
    # 최적 기간 찾기
    if not results: return None, None, None, "매매 기회가 없었습니다."
...
# [메인 UI] 탭 구성
# -----------------------------------------------------------
st.title("💸 Easy Swing Trader v16.0 (3-Tab)")
...
with tab3:
    ...
    st.caption("과거 데이터를 바탕으로 이 종목의 성격과 수익을 극대화하는 보유 기간을 계산합니다.")
    
    target_name = st.text_input("종목명을 입력하세요 (예: 삼성전자)", placeholder="삼성전자")
    
    if st.button("🔍 분석 시작") and target_name:
        with st.spinner(f"'{target_name}'의 2년치 데이터를 분석 중입니다..."):
            char_type, results, best_period, err = analyze_holding_period(target_name)
            
            if err:
                st.error(err)
            else:
                st.success(f"분석 완료: **{target_name}**")
                st.divider()
                
                # 1. 종목 성격
                st.subheader("1️⃣ 종목 특성 분석")
                st.info(f"이 종목은 **[{char_type}]** 입니다.")
                
                # 2. 결과 차트
                st.subheader("2️⃣ 보유 기간별 평균 수익률")
                
                # 막대 차트 생성
                colors = ['#e0e0e0'] * len(results)
                max_idx = list(results.keys()).index(best_period)
                colors[max_idx] = '#ff4b4b' # 1등은 빨간색
                ...
                fig.update_layout(height=300, yaxis_title="평균 수익률 (%)")
                st.plotly_chart(fig, use_container_width=True)
                
                # 3. 최종 제안
                st.subheader("3️⃣ AI의 제안")
                st.success(f"💡 **데이터 결론:** 이 종목은 매수 후 **[{best_period}]** 구간에서 매도했을 때 성과가 가장 좋았습니다.")
                if "모멘텀" in char_type:
                    st.caption("👉 변동성이 큽니다. 줄 때 먹고 나오는 단기 전략이 유리합니다.")
                elif "누적형" in char_type:
                    st.caption("👉 추세가 좋습니다. 너무 일찍 팔지 말고 진득하게 가져가세요.")
                else:
                    st.caption("👉 방향성이 없습니다. 박스권 매매나 뉴스 매매가 유효합니다.")
